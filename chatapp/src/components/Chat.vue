...
const onEnter = () => {
  const name = userName.value || "Unknown User";
  socket.emit("enterEvent", name.value);
}
...
const onPublish = () => {
  const name = userName.value || "Unknown User";
  socket.emit("publishEvent", `${name.value}さん:${chatContent.value}`)
  // 入力欄を初期化
  chatContent.value="";
}
...
const onExit = () => {
  const name = userName.value || "Unknown User";
  socket.emit("exitEvent", `${name.value}さんが退室しました。`)
}
